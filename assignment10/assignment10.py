...
def insert_user_func():
    ...
    #insert
    query = "insert into users(First_name,Last_name,email,password) values ('%s','%s','%s','%s')"%(fname,lname,email,password)
...
def Update_users_func():
    ...
    email = request.form['email']
    query = "UPDATE users SET First_name='%s',email='%s' WHERE id='%s';" % (name, email, user_id)
    interact_db(query=query, query_type='commit')
    return redirect('assignment10')
